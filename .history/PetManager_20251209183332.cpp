...
void PetManager::addPet() {
    ...
    cout << "Loai thu cung (co dau cach duoc): ";
    getline(cin, species);                    // ← BÂY GIỜ NHẬP "Chó Ta" ĐƯỢC!

    cout << "Ten thu cung            : ";
    getline(cin, name);                       // ← "Mèo Tam Thể Siêu Cute"

    cout << "Tuoi                     : ";
    while (!(cin >> age)) {
        cout << "Tuoi phai la so! Nhap lai: ";
        cin.clear();
        cin.ignore(10000, '\n');
    }
    cin.ignore(); // xóa ký tự \n sau khi nhập số

    cout << "Mau long (co dau cach duoc): ";
    getline(cin, color);                      // ← "Trắng Xám"

    cout << "Giong (co dau cach duoc)  : ";
    getline(cin, breed);                       // ← "Alaska Nâu Đỏ"

    cout << "Chu nhan (co dau cach duoc): ";
    getline(cin, owner);                      // ← "Nguyễn Văn Nam"
...
void PetManager::editPet() {
    system("cls");
    if (pets.empty()) {
        cout << "Danh sach trong! Khong co thu cung nao de sua.\n\n";
        system("pause");
        return;
    }
    ...
    if (!input.empty()) {
        try {
            ...
            if (newAge >= 0 && newAge <= 300) {
                ...
                cout << "Tuoi khong hop le (0-300), giu nguyen.\n";
            }
        } catch (...) {
            cout << "Tuoi khong hop le, giu nguyen.\n";
        }
    }

    cout << "Mau long   [" << pet->getColor()   << "]: ";
    getline(cin, input);
    if (!input.empty()) pet->setColor(input);

    cout << "Giong      [" << pet->getBreed()   << "]: ";
    getline(cin, input);
    if (!input.empty()) pet->setBreed(input);

    cout << "Chu nhan   [" << pet->getOwner()   << "]: ";
    getline(cin, input);
